...
$stbc_opts       = \STBC\admin\Settings::get();
$stbc_post_types = $stbc_opts['post_types'] ?? ['post', 'page'];
// phpcs:disable WordPress.DB.SlowDBQuery.slow_db_query_meta_key, WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- Dashboard summary query against plugin-managed backup meta.
$stbc_backed_query = new WP_Query([
    'post_type'              => $stbc_post_types,
    'post_status'            => 'publish',
    'meta_query'             => [
        [
...
$stbc_total_backed = (int) $stbc_backed_query->found_posts;

$stbc_is_pro = defined('STBP_VERSION');
?>
<div class="wrap" style="max-width:800px;">
    ...
    <div style="display:grid; grid-template-columns:repeat(2, 1fr); gap:16px; margin:24px 0;">
        <div style="background:#fff; padding:24px 16px; border:1px solid #ddd; border-radius:4px; text-align:center;">
            <div style="font-size:36px; line-height:1; font-weight:bold; color:#46b450; margin-bottom:8px;"><?php echo esc_html($stbc_total_backed); ?></div>
        ...
        <div style="background:#fff; padding:24px 16px; border:1px solid #ddd; border-radius:4px; text-align:center;">
            <div style="font-size:14px; line-height:1.4; font-weight:600; color:#444; margin-bottom:8px;"><?php
                foreach ($stbc_post_types as $stbc_i => $stbc_pt) {
                    $stbc_obj = get_post_type_object($stbc_pt);
                    echo '<span>' . esc_html($stbc_obj ? $stbc_obj->labels->singular_name : $stbc_pt);
                    if ($stbc_i < count($stbc_post_types) - 1) echo ', </span>'; else echo '</span>';
                }
            ?></div>
